      ...
      <div class="row ">
        <div class="col-md-2">
          <div class="flex flex-row items-center space-x-2 text-lg whitespace-nowrap">
                        <span style="color:white ;margin-top:10px;margin-left : 10px">{{wethVault.name}}s</span>
        ...
        <div class="px-2 py-4 sm:p-8">
            ...
            <div class="grid grid-cols-4 pb-4 px-4 text-sm  text-secondary "  >
                <div>Vault ID</div>
                <div class="flex items-center justify-end">Collateral (WETH)</div>
                      ...
                      <div class="flex-col space-y-2">
                          <div class="row">
                              ...
                              <div class="col-md-2 flex items-center">
                                  <div class="text-lg text-white overflow-hidden overflow-ellipsis"><span class="pull-right">{{parseFloat(vault.vaultCollateral).toFixed(2) }} WETH</span></div>
                              </div>
                          </div>
                          <div class="row">
            ...
            <!-- <div style="color:white;background: #0f1416 ; padding :15px; border-radius: 20px ; margin: 10px;" class="grid grid-cols-4 pb-4 px-4 text-sm  " v-for="vault  in vaults" :key="vault.id"  @click="openVault(vault.id)">
              ...
              <div class="flex items-center justify-end">{{parseFloat(vault.ratio).toFixed(2)}}</div>
                ...
                <div class="rounded-lg bg-yellow-400 bg-opacity-20 p-4 mt-4"><span class="text-sm text-white flex flex-row text-center">{{formatAmount(wethVault.gdaiAvailable)}} gDai available to mint<span style="margin-left: 4px;">
...
export default {
  data(){
      return {
        ...
        headers: [
                        ...
                        { text: 'Collateral (WETH)', value: 'Collateral' },
   ...
   vaults : function(){
    return this.$store.state.vault.wethVaults;
   }
    },
  watch: {
  ...
  methods : {
    ...
    openMonitor(){
      this.$router.push({ path: "/wethmonitor" })
   ...
   async searchVault(){
    this.loading= true;
      ...
      let valid = await window.wethVaultContract.methods.vaultExistence(this.valtId).call();
      if(valid){
         this.loading=  false;
    ...
    async loadVault(){
          let useraddress = this.user.address.toLowerCase();
      let body =  { 
                query: `
                    query {
      ...
      const result =  await axios.post("https://api.thegraph.com/subgraphs/name/blockifi/bsctetnetwethvault" ,body
      )
      console.log(result);
  ...
  if(users.length > 0){
    ...
    for(let index = 0; index < vaultIDs.length ; index++){
       ...
       vault.debt = await window.wethVaultContract.methods.vaultDebt(vaultIDs[index].id).call();
       vault.debt = window.web3.utils.fromWei(vault.debt);
       vault.vaultCollateral = await window.wethVaultContract.methods.vaultCollateral(vaultIDs[index].id).call();
     ...
     this.$store.dispatch("vault/setWethVaults" ,vaults).then(()=> {
     ...
     async rescanVaults(){
      console.log("vaultcount 1");
       ...
       for(let index = 0 ; index < vaultcount; index++){
         let vaultOwner =  await window.wethVaultContract.methods.vaultOwner(index).call();
